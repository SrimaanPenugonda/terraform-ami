...
resource "null_resource" "provisioner" {
  provisioner "remote-exec" { // connect and execute commands in remote instance
   ...
   # now using ansible
      ...
      "ansible-pull -i localhost, -U https://github.com/SrimaanPenugonda/Ansible.git roboshop-project/roboshop.yml -e ENV=${var.ENV} -e component=${var.COMPONENT} -e APP_ARTIFACT_VERSION=${var.ARTIFACT_VERSION} -e PAT=${jsondecode(data.aws_secretsmanager_secret_version.creds.secret_string)["PAT"]} -t ${var.COMPONENT} "
    ]
  }
}
